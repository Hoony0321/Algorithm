...
void SwappingBlock(int direction,int y, int x , vector<vector<int>>& map){
  switch(direction){
    //-----위-----//
    case 1:
      if(y-1 >= 0){ //맨 위에 있는 경우가 아닌 경우
        if(map[y-1][x] == 0){ // 빈 공간일 경우
          map[y-1][x] = map[y][x];
          map[y][x] = 0;
          SwappingBlock(1,y-1,x,map); //위에 또 빈 공간일수도 있으므로 실행
        }
        else{ //빈 공간일 아닐 경우
    ...
    case 2:
      if(y+1 < map_size){ //맨 아래에 있는 경우가 아닌 경우
        if(map[y+1][x] == 0){ // 빈 공간일 경우
          map[y+1][x] = map[y][x];
          map[y][x] = 0;
          SwappingBlock(2,y+1,x,map); //위에 또 빈 공간일수도 있으므로 실행
        }
        else{ //빈 공간일 아닐 경우
          if(map[y+1][x] == map[y][x]){ //같을 경우
            map[y+1][x] *= 2;
    ...
    case 3:
      if(x-1 >= 0){ //맨 왼쪽에 있는 경우가 아닌 경우
        if(map[y][x-1] == 0){ // 빈 공간일 경우
          map[y][x-1] = map[y][x];
          map[y][x] = 0;
          SwappingBlock(3,y,x-1,map); //위에 또 빈 공간일수도 있으므로 실행
        }
        else{ //빈 공간일 아닐 경우
    ...
    case 4:
      if(x+1 < map_size){ //맨 오른쪽에 있는 경우가 아닌 경우
        if(map[y][x+1] == 0){ // 빈 공간일 경우
          map[y][x+1] = map[y][x];
          map[y][x] = 0;
          SwappingBlock(4,y,x+1,map); //위에 또 빈 공간일수도 있으므로 실행
        }
        else{ //빈 공간일 아닐 경우
          if(map[y][x+1] == map[y][x]){ //같을 경우
            map[y][x+1] *= 2;
  ...
  switch(direction){
    ...
    case 1:
      for(int i = 0; i < map_size; i++){
        for(int j = 0; j < map_size; j++){
          if(map[j][i] != 0){
            SwappingBlock(direction, j, i,map);
          }
        }
      }
      break;
    ...
    case 3:
      for(int i = 0; i < map_size; i++){
        for(int j = 0; j < map_size; j++){
          if(map[i][j] != 0){
            SwappingBlock(direction, i, j,map);
          }
        }
      }
      break;
